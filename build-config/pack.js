...
const rendererMiniPlayerConfig = './renderer-mini-player/webpack.config.prod'
...
function build() {
  ...
  spinners.add('renderer-mini-player', { text: 'renderer-mini-player building' })
  ...
  Promise.all([
    ...
    pack(rendererMiniPlayerConfig).then(result => {
      results += result + '\n\n'
      spinners.succeed('renderer-mini-player', { text: 'renderer-mini-player build success!' })
    }).catch(err => {
      spinners.fail('renderer-mini-player', { text: 'renderer-mini-player build fail :(' })
      console.log(`\n  ${errorLog}failed to build renderer-mini-player process`)
      console.error(`\n${err}\n`)
      process.exit(1)
    }),
