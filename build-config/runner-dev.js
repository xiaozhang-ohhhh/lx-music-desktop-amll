...
const rendererMiniPlayerConfig = require('./renderer-mini-player/webpack.config.dev')
...
let hotMiddlewareRendererMiniPlayer
...
function startRendererMiniPlayer() {
  return new Promise((resolve, reject) => {
    const compiler = webpack(rendererMiniPlayerConfig)
    hotMiddlewareRendererMiniPlayer = webpackHotMiddleware(compiler, {
      log: false,
      heartbeat: 2500,
    })
    ...
    compiler.hooks.compilation.tap('compilation', compilation => {
      HtmlWebpackPlugin.getHooks(compilation).beforeEmit.tapAsync('html-webpack-plugin-after-emit', (data, cb) => {
        hotMiddlewareRendererMiniPlayer.publish({ action: 'reload' })
        cb()
      })
    })
    ...
    const server = new WebpackDevServer({
      ...
      setupMiddlewares(middlewares, devServer) {
        devServer.app.use(hotMiddlewareRendererMiniPlayer)
        setImmediate(() => {
          devServer.middleware.waitUntilValid(resolve)
        })
        return middlewares
...
function init() {
  ...
  spinners.add('renderer-mini-player', { text: 'renderer-mini-player compiling' })
  ...
  Promise.all([
    ...
    startRendererMiniPlayer().then(() => handleSuccess('renderer-mini-player')).catch((err) => {
      console.error(err.message)
      return handleFail('renderer-mini-player')
    }),
