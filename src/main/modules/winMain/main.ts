...
import { getPlatform, isLinux, isMac, isWin } from '@common/utils'
...
export const createWindow = () => {
   ...
   * 
   * 问题原因：
   * - Mac 上非透明窗口（transparent=false）的 roundedCorners 不会生效
   * - 打包后 dt 可能为 true，导致 transparent=false，圆角消失
   * 
   * 解决方案：
   * - Mac 上强制启用透明窗口，确保 roundedCorners 生效
   * - Windows/Linux 保持原有逻辑不变
   */
  let disableTransparent = global.envParams.cmdParams.dt
  
  // Mac 上强制启用透明窗口，修复打包后圆角消失
  if (isMac && disableTransparent) {
  ...
  void browserWindow.loadURL(winURL + `?os=${getPlatform()}&dt=${disableTransparent ? '1' : '0'}&dark=${shouldUseDarkColors}&theme=${encodeURIComponent(JSON.stringify(theme))}`)
