...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fontSizeUp = step => {
      const max = isFullscreen() ? 9999 : 200
      if (appSetting['playDetail.style.fontSize'] >= max) return
      setPlayDetailLyricFont(Math.min(appSetting['playDetail.style.fontSize'] + step, max))
    }
