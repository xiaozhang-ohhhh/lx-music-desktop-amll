...
export default () => {
  const applyWindowSize = (id: LX.AppSetting['common.windowSizeId']) => {
    const fallback = windowSizeList[2] ?? windowSizeList[0]
    const info = id == null
      ? fallback
      : (windowSizeList.find(i => i.id == id) ?? windowSizeList[0])
    const offset = (window?.lx?.rootOffset ?? 0) * 2
    setWindowSize(info.width + offset, info.height + offset)
  }

  watch(() => appSetting['common.windowSizeId'], (id) => {
  ...
  watch(isFullscreen, (fullscreen) => {
    ...
    applyWindowSize(appSetting['common.windowSizeId'])
  })
