...
export const search = async(text: string, page: number, sourceId: LX.OnlineSource | 'all'): Promise<LX.Music.MusicInfo[]> => {
  ...
  if (sourceId == 'all') {
    ...
    listInfo!.key = key
    let task = []
    const createSearchTask = (source: LX.OnlineSource, retries = 2): Promise<SearchResult> => {
      const attempt = (attemptCount: number): Promise<SearchResult> => {
        return (music[source]?.musicSearch.search(text, page, listInfos.all.limit) ?? Promise.reject(new Error('source not found: ' + source)))
          .catch(() => {
            if (attemptCount < retries) return attempt(attemptCount + 1)
            return {
              allPage: 1,
              limit: 30,
              list: [],
    ...
    for (const source of sources) {
      ...
      task.push(createSearchTask(source))
    }
    ...
    listInfo!.noItemLabel = window.i18n.t('list__loading')
    listInfo!.key = key
    const retrySearch = async (sourceId: LX.OnlineSource, retries = 2): Promise<SearchResult> => {
      const attempt = (attemptCount: number): Promise<SearchResult> => {
        return music[sourceId].musicSearch.search(text, page, listInfo!.limit)
          .catch(() => {
            if (attemptCount < retries) return attempt(attemptCount + 1)
            resetListInfo(sourceId)
            listInfo!.noItemLabel = '搜索失败，稍后重试'
            return {
              allPage: 1,
    ...
    const data = await retrySearch(sourceId)
