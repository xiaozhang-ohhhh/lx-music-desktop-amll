...
export const allHotKeys = markRaw({
  ...
  global: [
    ...
    {
      name: hotKeys.HOTKEY_MINI_PLAYER.toggle_visible.name,
      action: hotKeys.HOTKEY_MINI_PLAYER.toggle_visible.action,
      type: APP_EVENT_NAMES.winMiniPlayerName,
    },
  ],
})
...
export const onNewMiniPlayerProcess = (listener: LX.IpcRendererEventListener): RemoveListener => {
  rendererOn(WIN_MAIN_RENDERER_EVENT_NAME.process_new_mini_player_client, listener)
  return () => {
    rendererOff(WIN_MAIN_RENDERER_EVENT_NAME.process_new_mini_player_client, listener)
  }
}
